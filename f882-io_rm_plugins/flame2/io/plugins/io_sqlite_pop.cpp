...
void IOSQLitePop::readPop(std::string path,
    void (*addInt)(std::string const&, std::string const&, int),
    void (*addDouble)(std::string const&, std::string const&, double)) {
  // open sqlite file
...
void IOSQLitePop::executeSQLite(std::string statement) {
  char *zErrMsg = 0;
...
void IOSQLitePop::initialiseData() {
...
std::string IOSQLitePop::createUpdateStatement(std::string const& agent_name,
...
void IOSQLitePop::writePop(std::string const& agent_name,
