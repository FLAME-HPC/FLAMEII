...
FLAME_AGENT_FUNCTION(func_Y_10X) {
  int x = FLAME.GetMem<int>("x_int");
  FLAME.SetMem<double>("y_dbl", (x * 10.0));
  return FLAME_AGENT_ALIVE;
}

FLAME_AGENT_FUNCTION(func_Z_Xp1) {
  int x = FLAME.GetMem<int>("x_int");
  FLAME.SetMem<double>("z_dbl", (x + 1.0));
  return FLAME_AGENT_ALIVE;
}

FLAME_AGENT_FUNCTION(func_Y_XpY) {
  int x = FLAME.GetMem<int>("x_int");
  double y = FLAME.GetMem<double>("y_dbl");
  FLAME.SetMem<double>("y_dbl", (x + y));
  return FLAME_AGENT_ALIVE;
}
...
FLAME_AGENT_FUNCTION(func_X_YpZ) {
  double y = FLAME.GetMem<double>("y_dbl");
  double z = FLAME.GetMem<double>("z_dbl");
  FLAME.SetMem<int>("x_int", static_cast<int>(y+z));
  return FLAME_AGENT_ALIVE;
}
